...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (702117,742039,779947,742008,742040,1525544,742007)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (702117,742039,779947,742008,742040,1525544,742007)
  and c.invalid_reason is null
...
WHERE YEAR(E.start_date) - P.year_of_birth >= 16 AND E.start_date >= DATEFROMPARTS(2020, 12, 11)
GROUP BY e.person_id, e.event_id
