...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (702664,702665,702666,702667,702668,702669,702670,702671,702672,702673,702676,702677,702678,702680,702681,722117,722118,722119,724904,724905,739902,778259,778260,778261,778262,778263,778264,780152,905418,905419,35894915,36118948,36118949,36126197,37003432,739902,780152)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (702664,702665,702666,702667,702668,702669,702670,702671,702672,702673,702676,702677,702678,702680,702681,722117,722118,722119,724904,724905,739902,778259,778260,778261,778262,778263,778264,780152,905418,905419,35894915,36118948,36118949,36126197,37003432,739902,780152)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37003517,779678)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (37003517,779678)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1525542,779413,1525540,742037,742036)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1525542,779413,1525540,742037,742036)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-48,P.START_DATE) AND A.START_DATE <= DATEADD(day,-14,P.START_DATE) ) cc 
...
WHERE YEAR(E.start_date) - P.year_of_birth >= 18 AND E.start_date >= DATEFROMPARTS(2020, 12, 18)
GROUP BY e.person_id, e.event_id
