...
resultsFolder <- "~/Documents/EumaeusPlotFiles"
sourceFolder <- "~/Documents/Eumaeus-project/Eumaeus/extras/calibrationCodes/calibrateLLR"
savePlotsFolder <- "~/Documents/EumaeusPlotFiles/finalPlots"

#source("E:/Shounak_R/Eumaeus/extras/calibrationCodes/calibrateLLR/type1ErrorPowerPlots.R")

source(paste0(sourceFolder, "/type1ErrorPowerPlots.R"))

databaseId <- "OptumEhr"
exposuresToPlot <- c(21219, 21221, 21215, 211983, 21184, 211833)
...
allResults <- readr::read_csv(paste0(resultsFolder, "/allType1ErrorAndPowerResults.csv"))
...
for(exposureId in exposuresToPlot) {
  ...
  if(exposureId %in% c(21221, 211983, 211833)) { #2nd column plots
...
for(exposureName in exposuresToPlotNames) {
  ...
  plType1ErrorUncalibrated[[length(plType1ErrorUncalibrated) + 1]] <- allPlots[[exposureName]]$Type1ErrorUncalibrated
...
plType1ErrorCalibratedCombined <- wrap_plots(plType1ErrorCalibrated, nrow = 3, ncol = 2) &
  theme(legend.position = "bottom",
        ...
        axis.title.x = element_text(size=overallFontSize),
        axis.title.y = element_text(size=overallFontSize, angle=90, margin=margin(r=10))) 
...
plType1ErrorCalibratedCombined <- plType1ErrorCalibratedCombined +
  plot_annotation(title = "Type 1 Errors across time, after calibration",
                  theme = theme(plot.title = element_text(size = overallFontSize, hjust = 0.5, margin=margin(b=20)))) &
...
ggsave(plType1ErrorCalibratedCombined,
       filename = paste0(savePlotsFolder, "/type1ErrorCalibrated.eps"),
       width=450,
       height=550,
       units="mm",
       dpi = 400)
...
ggsave(plPowerCombined,
       filename = paste0(savePlotsFolder, "/power2.eps"),
       width=450,
       height=550,
       units="mm",
       dpi = 400)

## Uncalibrated Type 1 Error

plType1ErrorUncalibratedCombined <- wrap_plots(plType1ErrorUncalibrated, nrow = 3, ncol = 2) &
  theme(legend.position = "bottom",
        legend.text = element_text(size=legendTextSize),
        legend.title = element_blank(),
        axis.title.x = element_text(size=overallFontSize),
...
plType1ErrorUncalibratedCombined <- plType1ErrorUncalibratedCombined +
  plot_annotation(title = "Type 1 Errors across time, before calibration",
                  theme = theme(plot.title = element_text(size = overallFontSize, hjust = 0.5, margin=margin(b=20)))) &
  labs(x = "Months", y = "Type 1 Error, Uncalibrated") 
...
print(plType1ErrorUncalibratedCombined)

ggsave(plType1ErrorUncalibratedCombined,
       filename = paste0(savePlotsFolder, "/type1ErrorUncalibrated.eps"),
       width=450,
       height=550,
       units="mm",
       dpi = 400)
...
sysErrorOutput <- readr::read_csv(paste0(resultsFolder, "/systematicErrorDensityData.csv"))
...
ggsave(combinedSysErrorPlot,
       filename = paste0(savePlotsFolder, "/systematicErrorAcrossDataSources.pdf"),
       width=360,
       height=188,
       units="mm",
       dpi = 400)
...
##### Now obtain plots using plotInfo (NULL DENSITY PLOTS)

allPlots <- list()
plotInfo <- readr::read_csv(paste0(resultsFolder, "/nullDistributionsOptumEHR.csv"))
...
ggsave(densPlot,
       filename = paste0(savePlotsFolder, "/sysErrorOptumEHR.pdf"),
       width=450,
       height=550,
       units="mm",
       dpi = 400)
...
ggsave(combinedT1EPlot,
       filename = paste0(savePlotsFolder, "/T1EAcrossDataSources.eps"),
       width=425,
       height=205,
       units="mm",
       dpi = 400)
...
ggsave(combinedPowerPlot,
       filename = paste0(savePlotsFolder, "/PowerAcrossDataSources.eps"),
       width=425,
       height=205,
       units="mm",
       dpi = 400)
